...
def get_ytdlp_options(
    ...
    options = {
        ...
        # Keep logs reasonably clean.
        "quiet": True,

        "no_warnings": False,

        # IMPORTANT:
        # Prevent any system/user yt-dlp configuration
        # from changing the requested format.
        "ignoreconfig": True,
...
def build_analysis(
    ...
    # Sort video from highest quality.
    video_formats.sort(
        key=lambda x: (
            x.get("height") or 0,
            x.get("width") or 0,
    ...
    # Sort audio from highest bitrate.
    audio_formats.sort(
        key=lambda x: (
            x.get("abr") or 0,
        ),
