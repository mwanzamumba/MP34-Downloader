...
def unlock_cookies(cookies_path):
    ...
    try:
        path_buffer = create_unicode_buffer(cookies_path)

        result = DWORD(
            rstrtmgr.RmRegisterResources(
                session_handle,
                1,
                byref(pointer(path_buffer)),
                0,
                None,
                0,
                None
        ...
        if proc_info_needed.value:
            print(
                ...
                "Attempting to shut down the locking process...",
                file=sys.stderr
            )
            ...
            print("Cookie database is not locked.")

    finally:
        rstrtmgr.RmEndSession(session_handle)


def install_cookie_unlock():
    ...
    def unlock_chrome(database_path, tmpdir):
        try:
            ...
            print(
                "Attempting to unlock browser cookies...",
                file=sys.stderr
            )
